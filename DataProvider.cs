...
namespace MyExample
{
    public class DataProvider
    {
            ...
            using (SqlDataAdapter sc = new SqlDataAdapter("SELECT * FROM Competitors", con))
            {
                ...
                competitors = (from DataRow dr in dt.Rows
                               select new Skier()
                               {
                                   ...
                                   //Time =  TimeSpan.Parse(dr["Time"].ToString())
                               }).ToList();
            }
            return competitors;
        }
        ...
        public DataTable Results()
        {
            ...
            {
                con.Open();
                SqlDataAdapter sc = new SqlDataAdapter("select Competitors.id, Competitors.firstname, Competitors.lastname, competitors.sex, competitors.time, teamtable.teamname " +
                    "from competitors join teamtable on competitors.teamid = teamtable.id; ", con);
